...
# For compatibility with older .NET uses CreateEncryptor + Transform*Block.
# Dependency: Git Bash (use openssl to workaround issue if .NET MD5 fails)

$TYPE6_SALT_LEN = 8
$TYPE6_MAC_LEN = 4

function Get-MD5Hash([Byte[]]$in_bytes) {
    $bash = 'C:\Program Files\Git\bin\bash.exe'
    $b64_str = [Convert]::ToBase64String($in_bytes)
    $md5_str = $b64_str | & $bash -c "tr -d '\r' | base64 -d | openssl dgst -md5 -c"
    $md5_bytes = ForEach($str in ((-split $md5_str)[1] -split ':')) {[Convert]::ToByte($str,16)}
    return $md5_bytes
}

function Get-AesEncryptor([Byte[]]$key_bytes) {
    $aes = [Security.Cryptography.AES]::Create()
    $aes.KeySize = 128 
    $aes.Mode = [Security.Cryptography.CipherMode]::ECB
    $aes.Padding = [Security.Cryptography.PaddingMode]::None
    return $aes.CreateEncryptor($key_bytes,$null)
}
...
function password_mac_generate([Byte[]]$encrypted_key_bytes, $master_key <# str #>, [Byte[]]$salt) {
    ...
    $password_md5_digest = Get-MD5Hash $password
    $auth_key = (Get-AesEncryptor $password_md5_digest).TransformFinalBlock($salt + [Byte[]](0,0,0,0,0,0,0,0),0,16)
...
function transform_bytes($in_bytes, $salt_bytes, $key_str) {
    ...
    $aes = Get-AesEncryptor (Get-MD5Hash $key_bytes)
    ...
    for ($x=0; $x -lt $in_bytes.Length; ++$x) {
        ...
        if ($x_mod_16 -eq 0) { # New Key for every block size
            ...
            $block_key = $aes.TransformFinalBlock($block_key,0,16)
        }
        $x_byte = $in_bytes[$x]
